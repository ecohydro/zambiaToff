...
```{r, eval = FALSE}
fnms <- full_path(full_path(p_bd, "act_veg"), 
                  dir(full_path(p_bd, "act_veg"), pattern = ".tif$"))
s <- stack(lapply(fnms, function(x) raster(x)))
sm <- mask(s, mgrid)

# keep for assessing impacts
writeRaster(sm, filename = full_path(p_bd, "ZA-vegremain.tif"), overwrite = TRUE)
...
plot(wgts_prop2, prop_prot, pch = "", ylab = "")
text(wgts_prop2,  prop_prot, labels = labs, cex = 0.75)
