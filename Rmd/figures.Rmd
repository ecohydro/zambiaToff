...
for(i in c("selected_impacts.R", "imp_sorter.R", "imp_plot2.R")) {
  source(fp(p_root, paste0("R/", i)))
}
```
...
imp_plot2(mzsoyr, i1, p_figures, "ideal_yield3", "", crops, 1.2, TRUE, 
         c("red", "blue1", "gold"))
...
## Figures
### Figure 1 - pure objectives
```{r, eval = FALSE}
load(fp(p_out, "mainresults2802.rda"))
...
# selected_impacts(pure, fp(p_out, ybdc$both$bcode)) / 100000
conv_stats[1, ] / conv_stats[3, ]

# impsr <- lapply(c(2, 5, 3, 7:9, 4), function(x) conv_stats[[x]])
impsr <- lapply(c(2, 3, 5, 4), function(x) conv_stats[[x]])
# impacts[ind %in% pure]
...
ylabs <- c(expression(paste("Converted area (", km^2, " X ", 10^-5, ")")), 
           "Carbon loss (Mt)", #"Priority", 
           expression(paste("Biodiversity Index X ", 10^-5)),
           expression(paste("Truck hours X ", 10^-5)))

# yscale <- c(1, 10^5, 1, 100000, 1, 1, 1, 1)
yscale <- rep(100000, 4)#, 10000, 10000)#, 1, 1, 1)
ftits <- list("Maximize Yields", "Minimize Carbon Loss", 
              "Minimize Biodiversity Loss", "Minimize Travel Cost")
# leg <- c("Converted")
yaxes <- list(seq(0, 16, 2), seq(0, 1200, 100), seq(0, 9, 1), 
              seq(0, 35, 5))

# plot
# tiff(fp(p_figures, "single_priorities2.tif"), height = 3.5, width = 7, 
png(fp(p_figures, "single_priorities3.png"), height = 3.5, width = 7, 
    units = "in", res = 300)
...
# overlap/NND impact plots (recover from commit before 28/2 if needed)
mgp <- c(1, 0.25, 0)
# for(i in 1:7) {
cx <- 0.6
for(i in 1:4) { # i <- 3
  ...
  with(pms2b[ind, ], {
    ...
    mtext("to neighbour (km)", side = 2, line = 1.2, cex = 0.4)
  })
}
par(xpd = NA)
...
### Figure 2 - yield-carbon efficiency frontier
```{r, eval = FALSE}
p <- data.frame(ybdc$both$params)
pare <- ybdc$both$optitab
p[, c("Y", "C", "BD" ,"COST")] <- round(p[, c("Y", "C", "BD" ,"COST")], 2)

# Y versus C tradeoffs only
yconly <- which(p[, "BD"] == 0 & p[, "COST"] == 0)

# p[yconly, ]
ycstats <- selected_impacts(yconly, fp(p_out, ybdc$both$bcode))
ycstats <- ycstats[, .(iter, conv_area, tot_C)]

# subset of sites to map
yc_ss <- c(1, 2, 6, 11, 21)
ycmins <- sapply(2:3, function(x) range(ycstats[[x]]))
(ycmins[2, 2] - ycstats[yc_ss, get("tot_C")]) / diff(ycmins[, 2]) 
round(ycstats[yc_ss, get("tot_C")] / ycmins[2, 2], 2)
# 1.00 0.69 0.46 0.41 0.39
round(ycstats[yc_ss, get("conv_area")] / ycmins[1, 1], 3)
# 1.000 1.006 1.034 1.057 1.129

# plot colors
colsyc <- colorRampPalette(c("red", "gold", "blue"))
colsyc <- colsyc(5)
colsyc[4] <- "skyblue"

png(fp(p_figures, "pareto_yc.png"), height = 7, width = 7, 
    units = "in", res = 300)
par(mar = c(6, 6, 3, 1))
plot(ycstats$conv_area / yscale[1], ycstats$tot_C / yscale[2], axes = FALSE, 
     xlab = "", ylab = "", pch = 16, col = "grey40", xlim = c(13.5, 15.5), 
     ylim = c(425, 1200), xaxs = "i", yaxs = "i", cex = 1.5)
points(ycstats$conv_area[yc_ss] / yscale[1], cex = 2,
       ycstats$tot_C[yc_ss] / yscale[2], pch = 16, col = colsyc)
# lines(ycstats$conv_area / yscale[1], ycstats$tot_C / yscale[2])
axis(1, at = seq(13.5, 15.5, 0.25), mgp = c(3, 0.5, 0), tcl = -0.2, 
     labels = sprintf("%.2f", seq(13.5, 15.5, 0.25)), las = 2, cex.axis = 1.5)
ys <- seq(400, 1200, 50)
# ys[-seq(2, 30, 3)]
yslab <- ys
# yslab[-seq(2, 32, 3)] <- ""
axis(2, at = ys, labels = yslab, las = 2, mgp = c(3, 0.5, 0), tcl = -0.2, 
     cex.axis = 1.5)
mtext(ylabs[1], side = 1, line = 4.75, cex = 1.5)
mtext(ylabs[2], side = 2, line = 3.5, cex = 1.5)
dev.off()

# maps for those sites, just for 0.25 increments
fnames <- imp_sorter(fp(p_out, ybdc$both$bcode), 
                     inds = yc_ss, fullnames = TRUE)
yc_r <- lapply(fnames, function(x) {  # x <- pure[1]
  DT <- cbind(fread(x), basedt[, .(x, y, convertible)])
  DTr <- dt_to_raster(DT, CRSobj)
})
yc_r2 <- lapply(yc_r, function(x) calc(x[[1:2]], sum))  # conv pixels
names(yc_r2) <- paste0("YC", 1:5)

# Plot maps
# plot(1:5, pch = 16, col = colsyc(5))
legtf <- rep(FALSE, 5)
for(i in 1:5) { # i <- 1
  par(bg = "transparent")
  imp_plot2(yc_r2[[i]], impacts = i1, fpath = p_figures, 
            fnm = paste0("ycplot_", i), crops = "", ftitle = "", 
            col = colsyc[i], legend = legtf[i], hold = FALSE, stats = FALSE, 
            cexscale = 0.4, marg = rep(0, 4))
}  
...
# pairwise overlaps
yc_over <- lapply(1:nrow(pms2b), function(x) {  # apply logic tests
  calc(stack(yc_r2[which(pms2b[x, ] == 1)]), sum)
})
names(yc_over) <- pms2b$cl

# Jaccard Index of overlaps
yc_over_a <- sapply(1:length(yc_over), function(x) { # x <- 4
  sapply(1:2, function(y) {  # y <- 3
    r <- yc_over[[x]] == y
    round(cellStats(r * pure_r[[1]]$convertible, sum), 1)
  })
})
pms2b$JS <- round(yc_over_a[2, ] / colSums(yc_over_a) * 100, 2)
...
cstats <- cstats[, .(iter, conv_area, tot_C, tot_cost, cons_tot)]
# colSums(outtab[1:2, c("conv_area", "tot_C", "tot_cost", "cons_tot")])
# pare[ind %in% pind]
...
# percent of total possible savings by the 15\% weight to both C and BD
((minmaxstats[2, .(conv_area, tot_C, cons_tot, tot_cost)]) - 
  (imp_pct[4, .(conv_area, tot_C, cons_tot, tot_cost)])) /
  imp_range[, .(conv_area, tot_C, cons_tot, tot_cost)]

cols_tr <- c(rgb(1, 0, 0), rgb(0, 0, 1), rgb(0, 0.75, 1), rgb(1, 0.75, 0))
# sctab <- do.call(rbind, scens) * 100
cx <- 0.7
imp_pct[, {
  png(fp(p_figures, "weight_impact5.png"), height = 4, width = 4, 
      units = "in", res = 300)
  plot(xwgts, conv_areap, type = "l", xlab = "", ylab = "", axes = FALSE, 
       ylim = c(0, 55), col = cols_tr[1])
  lines(xwgts, tot_Cp, col = cols_tr[2])
  lines(xwgts, cons_totp, col = cols_tr[3])
  lines(xwgts, tot_costp, col = cols_tr[4])
  ...
  legend("bottomright", 
         legend = c("Yield", "Carbon", "Biodiversity", "Travel cost"), 
...
minmaxstats[1, ] / 10000 * 1.05 - minmaxstats[1, ] / 10000
...
impsr <- lapply(c(2, 3, 5, 4), function(x) all_stats[[x]])
...
maxes <- sapply(c(2:3, 5, 4), function(j) minmaxstats[[j]][2])
mins <- sapply(c(2:3, 5, 4), function(j) minmaxstats[[j]][1])
ymaxes <- maxes / yscale
ymins <- mins / yscale
...
ftits <- list("Compromise 5% on Yield", "Compromise 5% on Carbon", 
              "Compromise 5% on Biodiversity", "Compromise 5% on Travel Cost")
yaxes <- list(seq(0, 16, 4), seq(0, 1200, 100), seq(0, 9, 1), 
              seq(0, 35, 5))
pcttxt <- c("best", "50%", "worst")
impsr

# plot
png(fp(p_figures, "compromise_5pct3.png"), height = 3.5, 
...
for(i in 1:4) { 
  ...
  with(pms2bb[ind, ], {
    ...
    mtext("to neighbour (km)", side = 2, line = 0.8, cex = 0.4)
  })
}
par(xpd = NA)
legx <- grconvertX(0.4, from = "ndc", to = "user")
...
png(fp(p_figures, "compromise_25pct2.png"), height = 2, width = 4, 
    units = "in", res = 300)
l <- layout(mat = lmat)
# layout.show(l)
...
par(oma = c(0, 0, 0, 1))
# plot(zambia)
imp_plot2(toresr$eqwt, impacts = i1, fpath = p_figures, fnm = "bob",
...
for(i in 1:4) {
  1 - print(all_stats[5, c(2:3, 5, 4), with = FALSE] / 
              all_stats[i, c(2:3, 5, 4), with = FALSE])
}

# equal compromise avoided cost
for(i in 1:5) {
  print(((minmaxstats[2, c(2:3, 5, 4), with = FALSE]) - 
     (all_stats[i, c(2:3, 5, 4), with = FALSE])) /
    imp_range[, c(2:3, 5, 4), with = FALSE])
}

# equal cost difference relative to best case
for(i in 1:5) {
  print(((all_stats[i, c(2:3, 5, 4), with = FALSE]) - 
     (minmaxstats[1, c(2:3, 5, 4), with = FALSE])) /
    imp_range[, c(2:3, 5, 4), with = FALSE])
}
...
fb_sim <- fb_over_a[3, ] / colSums(fb_over_a) * 100
...
for(i in 1:2) {
  ...
  mtext(paste("% Overlap =", round(fb_sim[i], 2)), side = 3, line = -2, 
        cex = 0.4, adj = 0.2)
  mtext(paste("Mean dist. to neighbour =", round(fb_nnd[i]), "km"), side = 3,
        line = -2.5, cex = 0.4, adj = 0.2)
}
dev.off()
...
bd <- dt_to_raster(cbind(basedt[, .(x, y)], il$cons), CRSobj)
...
for(i in 1:2) {
  ...
  plot(ylds[[i]], legend = TRUE, add = TRUE, axis.args = list(cex.axis = 0.7),
       legend.args = list(text = expression("tons ha"^-1), cex = 0.8))
  plot(pas[pas@data$type %in% c("gma", "npark"), ], add = TRUE, col = pacols[3],
       border = pacols[3])
  if(i == 1) {
    legend("bottomright", legend = c("Protected Areas"),
           pch = 15, col = c(pacols[2]), bty = "n",
           pt.cex = 2, cex = 0.8, inset = c(0, 0.1, 0, 0))
  }  
}
dev.off()
...
par(mar = c(0, 0, 0, 5))
plot(zambia, col = "white", border = "transparent")
plot(closs, legend = TRUE, add = TRUE, axis.args = list(cex.axis = 0.7),
       legend.args = list(text = expression("tons ha"^-1), cex = 0.8))
plot(pas[pas@data$type %in% c("gma", "npark"), ], add = TRUE, col = pacols[2],
     border = pacols[2])
legend("bottomright", legend = c("Protected Areas"),
       pch = 15, col = c(pacols[2]), bty = "n",
       pt.cex = 2, cex = 0.8, inset = c(0, 0.1, 0, 0))
...
plot(cost, legend = TRUE, add = TRUE, 
     ...
     legend.args = list(text = "hours", cex = 0.8))
plot(pas[pas@data$type %in% c("gma", "npark"), ], add = TRUE, col = pacols[2],
     border = pacols[2])
legend("bottomright", legend = c("Protected Areas"),
       pch = 15, col = c(pacols[2]), bty = "n",
...
png(fp(p_figures, "bd.png"), height = 3.5, width = 4.5, unit = "in", 
     res = 300)
par(mar = c(0, 0, 0, 5))
plot(zambia, col = "white", border = "transparent")
plot(bd, legend = TRUE, add = TRUE, axis.args = list(cex.axis = 0.7))
...
# vegetation types
load(fp(p_bd, "vegstats.rda"))
vegstats <- round(vegstats * 100, 2)
nms <- c("Afromontane rain forest", "Zambezian dry evergreen forest", 
         "Zambezian dry deciduous forest", "Palm wooded grassland",
         "Riverine wooded vegetation", "Zambezian Kalahari woodland",
         "Drier miombo woodland", "Miombo woodland on hills/outcrops",
         "Wetter mimbo woodland", "N. Zambezian undifferentiated woodland",
         "Catena of N. Zambezian undifferentiated woodland/edaphic grassland", 
         "Mopane woodland/scrub woodland", "Zambezian chipya woodland",
         "Itigi thicket", "Swamp forest",
         "Edaphic grassland on drainage impeded/seasonally flooded soils", 
         "Bush groups, typically around termitaria",
         "Edaphic grassland w/Zambezian Kalahari woodland",
         "Edaphic grassland w/freshwater swamp", 
         "Edaphic woodland on drainage impeded/seasonally flooded soils")
print(xtable::xtable(cbind(c("", nms), unname(t(vegstats)[, c(1:2, 4, 3, 5)])), 
                           digits = c(2, 2, 2, 1, 1, 1, 1), ))
...
carbon <- brick(fp(p_root, "external/input_devel/carbon/ZA-carbon.tif"))
carbont <- calc(carbon, sum)
carbont <- raster::mask(carbont, mgrid, maskvalue = 0)
freq(!is.na(carbont))  # 738442 km2
quantile(carbont, prob = seq(0, 1, 0.1))  # 50% 130 t/ha, 80 t/ha
