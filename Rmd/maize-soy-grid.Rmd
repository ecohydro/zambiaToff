...
yld_rl <- lapply(crops, function(x) {  # x <- crops[[1]]
  yr <- lapply(c("S", "R", "I"), function(y) {  #y <- "S"
    ...
    r <- dt_to_raster(ylds[, c("x", "y", cnms), with = FALSE], CRSobj = gcs)
...
This provides the methods for taking 10 km grids, averaging them across cultivars within management treatments, and then reseampling to 1 km grids. This is approximately the approach that will be used to make inputs for the tradeoff model. Other methods to explore include blending different types of management practice, e.g. X% of each cell is irrigated, Y% is rainfed commercial, Z% is subsistence. 
```{r, eval=FALSE}
