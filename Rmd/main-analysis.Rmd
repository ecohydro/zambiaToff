...
imp_plot2(dtr = rs_over_rc2, impacts = i1, fpath = p_figures, 
          fnm = "pure_overlap", cexscale = 0.5, ftitle = "", crops = leg2, 
...
plot(1:3, pch = 20, col = cols, cex = 20)
...
tiff(fp(p_figures, "changing_weights.tif"), height = 4, width = 7, 
     units = "in", res = 300)
par(mfrow = c(2, 4), oma = c(0, 0, 1, 0))
for(i in 1:4) { # i <- 1
  # map
  imp_plot2(scens_over[[i]], impacts = i1, fpath = p_figures, fnm = "bob",
            crops = leg[c(1, 3, 2)], ftitle = ftits[[i]], col = cols, 
...
# tores_over_a <- lapply(1:nrow(pms2a), function(x) {  # apply logic tests
#   (toresr[[1]] == pms2a[x, 1]) & (toresr[[2]] == pms2a[x, 2]) & 
#     (toresr[[3]] == pms2a[x, 3]) & (toresr[[4]] == pms2a[x, 4])
# })
# names(tores_over_a) <- pms2a$cl

# colst <- c(rgb(1, 0, 0),   # Ag
#            rgb(1, 0.5, 0),  # Ag 2
#            rgb(1, 1, 0),   # Ag1 and 2
#            rgb(0, 0, 1),  # Obj
#            rgb(1, 0, 1),  # Ag1 and Obj
#            rgb(1, 0.5, 1),  #  Ag2 and Obj
#            rgb(1, 1, 0.5),  #  Ag, Ag2 and Obj
#            rgb(0, 0.5, 1),  # Subj
#            rgb(0, 1, 1),  # Ag and Subj
#            rgb(0, 1, 1),  # Ag2 and Subj
#            rgb(0, 1, 1),  # Ag, Ag2, Subj
#            rgb(0, 1, 1),  # Obj and Subj
#            rgb(0, 1, 1),  # Ag,  Obj and Subj
#            rgb(0, 1, 1),  # Ag2,  Obj and Subj
#            rgb(0, 1, 1))  # All
# plot(1:length(colst), pch = 20, cex = 3, col = colst)
...
# plot(toresr$ag2)
# plot(tores$obj$map - tores$subj$map)

# cols_tr <- c(rgb(1, 0.5, 0, alpha = 0.5), rgb(1, 0, 0, alpha = 0.5), 
#              rgb(0, 0.5, 1, alpha = 0.5), rgb(0, 0, 1, alpha = 0.5))
cols_tr <- c(rgb(1, 0, 0), rgb(1, 0.75, 0), rgb(0, 0, 1), rgb(0, 0.75, 1))
plot(1:4, cex = 3, pch = 20, col = cols_tr)

# par(mfrow = c(2, 2), mar = rep(0, 4))
# # plot(tores$ag$map, col = c("grey", cols_tr[1]), axes = FALSE, legend = FALSE)
# for(i in 1:4) {
#   plot(zambia, col = "grey")
#   plot(toresr[[i]], col = c("transparent", cols_tr[i]), add = TRUE) 
# }
